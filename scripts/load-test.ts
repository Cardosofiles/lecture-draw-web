...
async function signCookieValue(value: string, secret: string) {
  ...
  const mac = await webcrypto.subtle.sign('HMAC', key, new TextEncoder().encode(value))
...
async function createVirtualUsers() {
  ...
  for (let i = 0; i < USERS; i++) {
    ...
    cookies.push(`better-auth.session_token=${await signCookieValue(token, secret)}`)
  }
  return cookies
}
...
async function main() {
  console.log(`▶  ${USERS} attendees · ${PATH} every ${INTERVAL}ms · ${DURATION}s · ${TARGET}`)
  console.log(`   expected offered load ≈ ${((USERS / INTERVAL) * 1000).toFixed(1)} req/s`)
  ...
  console.log(`success       ${ok}/${total}  (${((ok / total) * 100).toFixed(2)}%)`)
