...
const { executeSQL, getSchemaBrowser, getQueryHistory } = await import(
  "@/actions/sql-console"
);
...
describe("executeSQL — authorization", () => {
  ...
  it("rejects a non-admin caller", async () => {
    ...
    await expect(executeSQL('SELECT * FROM "User"')).rejects.toThrow(
      /Acesso negado/i,
    );
...
describe("executeSQL — destructive-statement guard", () => {
  ...
  it("REGRESSION: blocks TRUNCATE without the TABLE keyword", async () => {
    ...
    await expect(executeSQL('TRUNCATE "User"')).rejects.toThrow(
...
describe("executeSQL — error translation and logging", () => {
  it("translates a missing-relation error to Portuguese", async () => {
    ...
    await expect(executeSQL('SELECT * FROM "Foo"')).rejects.toThrow(
      /Tabela não encontrada/i,
    );
  });
